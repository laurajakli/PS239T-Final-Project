###This contains the entire code for data analysis and visualization for Jill Stein.
...
df <- read.csv("drjillstein_facebook.csv", fileEncoding="latin1")
...
pdf("top_angrystatus_Stein.pdf", height=12, width=48)
grid.table(dfangry)
dev.off()
...
reactionsplot <- ggplot(df_agg_long %>% filter(reaction!="total_likes"), aes(x=date, y=count, color=reaction, fill=reaction)) +
  ...
  labs(title="Daily Breakdown of Reactions on Jill Stein's FB Posts",
       x="Date of Status",
...
jill <- read.csv('drjillstein_facebook.csv', stringsAsFactors = FALSE)
...
jillCorpus <- Corpus(VectorSource(jill$status_message))
...
jillCorpus<- tm_map(jillCorpus,
                    content_transformer(function(x) iconv(x, to='UTF-8-MAC', sub='byte')),
                    mc.cores=1)
jillCorpus <- tm_map(jillCorpus, PlainTextDocument)
jillCorpus <- tm_map(jillCorpus, removePunctuation)
jillCorpus <- tm_map(jillCorpus, removeNumbers)
jillCorpus <- tm_map(jillCorpus, stemDocument)
jillCorpus <- tm_map(jillCorpus, removeWords, c('the', 'this', 'a', stopwords('english')))
wordcloud(jillCorpus, scale=c(3,0.5), max.words = 100, random.order = FALSE, 
          colors=brewer.pal(8, 'Dark2'))


#Here, we create a Document-Term Matrix (DTM)

dtm <- TermDocumentMatrix(jillCorpus)
m <- as.matrix(dtm)
