###This contains the entire code for data analysis and visualization for Hillary Clinton.
...
df <- read.csv("hillaryclinton_facebook.csv", fileEncoding="latin1")
...
pdf("top_angrystatus_Clinton.pdf", height=8, width=28)
grid.table(dfangry)
dev.off()
...
reactionsplot <- ggplot(df_agg_long %>% filter(reaction!="total_likes"), aes(x=date, y=count, color=reaction, fill=reaction)) +
  ...
  labs(title="Daily Breakdown of Reactions on Hillary Clinton's FB Posts",
       x="Date of Status",
...
hildog <- read.csv('hillaryclinton_facebook.csv', stringsAsFactors = FALSE)
...
hilCorpus <- Corpus(VectorSource(hildog$status_message))
...
hilCorpus <- tm_map(hilCorpus,
                    content_transformer(function(x) iconv(x,"latin1", "ASCII", sub="")),
                    mc.cores=1)
hilCorpus <- tm_map(hilCorpus, PlainTextDocument)
hilCorpus <- tm_map(hilCorpus, removePunctuation)
hilCorpus <- tm_map(hilCorpus, removeNumbers)
hilCorpus <- tm_map(hilCorpus, stemDocument)
hilCorpus <- tm_map(hilCorpus, removeWords, c('the', 'this', 'a', stopwords('english')))
wordcloud(hilCorpus,scale=c(5,0.5),  max.words = 100, random.order = FALSE, 
          colors=brewer.pal(8, 'Dark2'))


#Here, we create a Document-Term Matrix (DTM)

dtm <- TermDocumentMatrix(hilCorpus)
m <- as.matrix(dtm)
