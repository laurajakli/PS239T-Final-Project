###This contains the entire code for data analysis and visualization for Donald Trump.
...
df <- read.csv("DonaldTrump_facebook.csv")
...
pdf("top_angrystatus_Trump.pdf", height=8, width=28)
grid.table(dfangry)
dev.off()
...
###There's one date that aggregated wrong in the Trump data
##(not with any other candidate) so I'm removing that first
##row withthecode below. 
...
# This function allows us to create the color schemes
# for our figures using RColorBrewer.
# First, we set the palette color for each element, 
# then set different elements of the chart (grid, axes, 
# legend, title, etc to these colors and fills.
...
reactionsplot <- ggplot(df_agg_long %>% filter(reaction!="total_likes"), aes(x=date, y=count, color=reaction, fill=reaction)) +
  ...
  labs(title="Daily Breakdown of Reactions on Donald Trump's FB Posts",
       x="Date of Status",
...
df <- read.csv("DonaldTrump_facebook.csv", fileEncoding="latin1")
...
donald <- read.csv('DonaldTrump_facebook.csv', stringsAsFactors = FALSE)
...
donCorpus <- Corpus(VectorSource(donald$status_message))
...
donCorpus <- tm_map(donCorpus,
                   content_transformer(function(x) iconv(x, to='UTF-8-MAC', sub='byte')),
                   mc.cores=1)
donCorpus<- tm_map(donCorpus, PlainTextDocument)
donCorpus <- tm_map(donCorpus, removePunctuation)
donCorpus <- tm_map(donCorpus, removeNumbers)
donCorpus <- tm_map(donCorpus, stemDocument)
donCorpus <- tm_map(donCorpus, removeWords, c('the', 'this', 'a', stopwords('english')))
wordcloud(donCorpus, scale=c(5,0.5), max.words = 100, random.order = FALSE,
          colors=brewer.pal(8, 'Dark2'))

#Here, we create a Document-Term Matrix (DTM)
dtm <- TermDocumentMatrix(donCorpus)
m <- as.matrix(dtm)
